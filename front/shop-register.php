<?php
  ...
  if((new SiteIdentity())->demo == false){
    if(isset($_POST['sign_up'])){
      ...
      $response = $user_info->save_user_login_info();
      if($response->status == 1){
        $user_info->go_to_redirect_url_after_sign_up();
      }
      if(!$response->status){
            ...
            <form role="form" class="form-horizontal form-without-legend" method="post">
                ...
                <div class="form-group">
                  ...
                  <div class="col-lg-8 checkbox-list">
                    <label>
                      <input style="margin-right: 10px;" type="checkbox"> I have read all the <a href="shop-terms-conditions-page.php">term and Conditions</a>.
                    </label>
                  </div>
                </div>
                <div class="row">
                  <div class="col-lg-8 col-md-offset-2 padding-left-0 padding-top-20">
                    <button class="btn btn-primary" type="submit" name="sign_up">SIGN UP NOW</button>
                    <p style="margin-top: 20px;">Alrady an account? <a href="shop-login.php?id=<?php echo $product_id ?>">Sign In</a>
                  </div>
                </div>
              </form>
            </div>
